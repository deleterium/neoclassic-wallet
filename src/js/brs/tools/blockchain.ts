...
export function drawBlocksInCurrentPage(blocks: BlockDetails[]) {
    ...
    for (const block of blocks) {
        ...
        const percentage =
            Math.round((Number(block.baseTarget) / 153722867) * 100)
                .toString()
                .padStart(4, '0') + ' %'
    ...
    if (BRS.currentPage === 'forged_blocks') {
        ...
        if (time === 0) {
            ...
            $('#latest_blocks_transactions_per_hour')
                .html(Math.round((totalTransactions / (time / 60)) * 60).toString())
        ...
        $('#latest_blocks_average_generation_time')
            .html(Math.round(time / 100) + 's')
            .removeClass('loading_dots')
    }
