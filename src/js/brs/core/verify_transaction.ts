...
export function verifyTransactionBytes(transactionBytes: string, signature: string, requestType: string, data: any) {
    ...
    function checkBaseTransaction() {
        if (
            transaction.senderPublicKey !== BRS.accountInfo.publicKey ||
        ...
        if (transaction.amountNQT !== data.amountNQT) {
            ...
            const requestTypeWithSeperatedAmountNQTCalculation = [
                'sendMoneyMulti',
                'sendMoneyMultiSame',
                'sendMoneyEscrow',
                'addCommitment',
        ...
        switch (rqType) {
            ...
            case 'addCommitment':
                return {
                    type: 20,
                    subtype: 1,
                    attachmentInfo: [{ type: 'Long*1', value: [data.amountNQT] }],
                }
            ...
            case 'sendMoneySubscription':
                return {
                    type: 21,
                    subtype: 3,
                    attachmentInfo: [{ type: 'Int*1', value: [data.frequency] }],
                }
            case 'subscriptionCancel':
                return {
                    type: 21,
                    subtype: 4,
                    attachmentInfo: [{ type: 'Long*1', value: [data.subscription] }],
                }
            default:
                return {
                    type: -1,
