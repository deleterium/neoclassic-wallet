...
export function convertPublicKeyFromBase36ToBase16(inVal: string) {
    function convert(value: string) {
        return [...value.toString()].reduce((r, v) => r * 36n + BigInt(parseInt(v, 36)), 0n)
    }
            ...
            switch (response.errorDescription) {
                ...
                default:
                    if (response.errorDescription.indexOf('Alias already owned by another account') !== -1) {
                        ...
                        response.errorDescription.match(
                            /Invalid effective balance leasing:.*recipient account.*not found or no public key published/,
                        )
                    ) {
